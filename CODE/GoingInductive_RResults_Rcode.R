#INDEX
# # Summary results all sample (with school effect) - 2003 to 2014 - 15
# School 1 - 2003 to 2014 - 50
# Summary results all sample (with school effect) - 2003 to 2014 - 85
# School 1 - 2003 to 2014 - 120
...
results <- data.frame(
  ...
  TEST1=c(-57,67,67,68,68,66,66,-37,58,80,67,67),
...
results <- data.frame(
  samples=c("all sample", "all sample (school effect)" , "JUNIOR (school effect)", "JUNIOR (school effect)(S1 with 4CID)", "GRADE 7 (school effect)","GRADE 8 (school effect)", "GRADE 9 (school effect)", "RANK 1 (school effect)", "RANK 2 (school effect)","RANK 3 (school effect)","FEMALE (school effect)","MALE (school effect)"),
  TEST1=c(62,62, 67,68,68,66,66,-43,58,80,67,67),
  TEST2=c(56,68, 70,69,68,70,70,-37,58,81,72,67),
  LAB1=c(60,60,63,62,63,60,60,30,57,77,63,62),    # NEXT:  "JUNIOR (school effect)"
  LAB2=c(63,76,79,78,78,75,75,45,66,88,83,76),
  BEHAV1=c(79,79,-77,-77,-77,-76,-80,76,-56,-72,-85,79,-77),
  BEHAV2=c(83,79,-73,-73,-73,-70,-69,82,-46,-64,-80,79,-70),
  CLASS1=c(64,64,68,69,69,67,68,42,61,78,68,68),
  CLASS2=c(64,72,72,71,73,70,73,42,63,81,75,69)
)
results
ggplot(results,aes(group=TEST1)) +
  geom_errorbar(aes(x=samples, ymin=TEST1, ymax=TEST2,color='TEST'), width = ifelse(results$TEST1==results$TEST2, 1, 0.3))+

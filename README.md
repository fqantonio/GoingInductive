...
This study utilized the **4C/ID instructional design framework**, aligned with Cognitive Load Theory (CLT) principles, for bLearning activities. CLT principles, such as worked examples, completion problems, and minimizing split-attention, were integrated. The 4C/ID approach was chosen due to its alignment with complex learning theories and its structured approach.

The theoretical framework for this study is Evolutionary Educational Psychology, which emphasizes the importance of integrating new information with existing knowledge structures (schemas) in long-term memory. This aligns with CLT's focus on minimizing working memory load. By considering students' prior knowledge and utilizing effective instructional strategies, the study aimed to facilitate schema development and optimize learning outcomes.

[continue reading...](https://github.com/fqantonio/GoingInductive/blob/main/THEORY)

# PROCEDURE

**In practice**, remembering the authors of 4C/ID analogy with an ocean liner (Van Merriënboer, Clark, Croock, 2002): "huge, slow, poderous, and recuring larges amounts of energy and a great deal of time to move...". All this work took a lot of effort, including all the prodedure underlying the pedagogic planning: reading, testing, reading and searching, and reading again. The **4C/ID** of Van Merriënboer has a steep learning curve but it provides a consistent lighthouse during all the process, meaning a consistent managment of instructional sequences before the LMS organization. This model reinforce, time after time, the connection between the theories underlying 4C/ID and his concrete struture, namely, task classes (TC), Learning Tasks (LT), part-task-practice (PTP), Supportive Information (SI), Procedural Information (PI) and just-in-time (JIT). This is all organizing taking in account the need for whole-tasks in a real-world-problem context learning situations that support gradual aqcquisition of skills and knowledge in a cognitively sustanaible manner, i.e., the limited processing capacity of working memory by gradually increasing the level of difficulty of the whole tasks. According to this methodology, LT for complex environments are integrated and run from simple to complex sequence of TC, with gradually dimishing levels of support/fading within each class (scafolding). Suficient SI is provided for general aspects of learning tasks, and JIT information engage students in automation for the learning goals. PTP are offered to train constituent skills that needed to be performed at a very high level of automaticity (Kalyuga, Paas et all, 2010) in order to overcome the restrictive working memory in more complex learning activities. 
Being that said, the course structure were organized using cmap tools (see example attached), a software online tool to develop mind maps about the overall learning structure with two dimentions:
1. In the horizontal, analog to a time-line, follows a concrete-to-abstract task classses (TC), each one having to promote the aquisition of higher level conceptual schemas and mental models.
2. 

About the load, the focus, during the planning, was on intrinsic and extraneous load rather then germane load, mostly because the goal was on the content rather then the student, that is, focus on making small learning steps with guidance and feedback capability in an variability problem environment. Germane load is considerered in CLT as the mental resources dedicated to processing, constructing, and automating schemas, so it was considered that the dual framework of intrinsic and extraneous load is sufficient and non-redundant, making the boundaries of the procedure not so overwhelming and more clearer for the concrete content creation and implementation (Kalyuga, 2011).
